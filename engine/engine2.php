...
if ($get == 'toplist') {
	...
	if ($category == 'versions') {
		...
		$h2Title = 'Minecraft '.$versionName['name'].' '.$txt['txt227'];
		$seoTitle = 'Minecraft '.$versionName['name'].' '.$txt['txt227'];
		...
		$h2Title = 'Minecraft '.$typeName['name'].' '.$txt['txt227'];
		$seoTitle = 'Minecraft '.$typeName['name'].' '.$txt['txt227'];
		...
		?>
    		<tr>
      			...
      			<td class="server">
  					...
  					<div class="server-ip">
                		<p><i class="plug icon"></i> <?php echo $host; ?></p>
...
?>
		<h2 class="ui large header"><?php echo $h2Title; ?></h2>
		<hr>
		...
		?>
    		<tr>
      			...
      			<td class="server">
  					...
  					<div class="server-ip">
                		<p><i class="plug icon" aria-hidden="true"></i> <?php echo $host; ?></p>
		                <div class="copy-action" data-clipboard-text="<?php echo $host; ?>">
		                    <button class="copy-text"><i class="cut icon" aria-hidden="true"></i> <?php echo $txt['txt9']; ?></button>
		                    <button class="copy-success" aria-label="<?php echo $txt['txt9']; ?>"><i class="thumbs up outline icon" aria-hidden="true"></i></button>
		                </div>
	...
	<div class="ui segments">
		...
		<div class="ui tall stacked segment stackable grid list">
			...
			<div class="eight wide column">
			  	<a class="fluid ui button" href="<?php echo $txt['SITE_LINK'].'/versions/'.strtolower(str_replace(' ', '', $name)); ?>" title="<?php echo safeInput($name).' Minecraft '.$txt['txt227']; ?>">
  					<?php echo $name; ?>
				</a>
			</div>
			<?php
...
?>
	<div class="ui segments">
		...
		<div class="ui tall stacked segment stackable grid list">
			...
			?>
			<div class="eight wide column">
			  	<a class="fluid ui button" href="<?php echo $txt['SITE_LINK'].'/types/'.strtolower(str_replace(' ', '', $name)); ?>" title="<?php echo safeInput($name).' Minecraft '.$txt['txt227']; ?>">
  					<?php echo $name; ?>
				</a>
			</div>
			<?php
  				...
  				<tbody id="server">
    				...
    				<tr>
						...
						foreach ($versions_extract[1] as $version) {
							echo '<a href="'.$txt['SITE_LINK'].'/versions/'.strtolower(str_replace(' ', '', $version)).'" class="ui label" title="'.safeInput($version).' Minecraft '.$txt['txt227'].'">'.safeInput($version).'</a>';	
						} 
						?>
						</td>
    				</tr>
    				...
    				<tr>
						...
						foreach ($types_extract[1] as $type) {
							echo '<a href="'.$txt['SITE_LINK'].'/types/'.strtolower(str_replace(' ', '', $type)).'" class="ui label" title="'.safeInput($type).' Minecraft '.$txt['txt227'].'">'.safeInput($type).'</a>';	
						} 
						?>
						</td>
    				</tr>
    	...
    	<div class="eleven wide stretched column">
			...
			<form class="ui small form" action="#<?php validate('add'); ?>" enctype="multipart/form-data" method="post">
				<h4 class="ui dividing header"><i class="plus square icon"></i> <?php echo $txt['txt86']; ?></h4>
		  		...
		  		<div class="required field">
		    		<label><?php echo $txt['txt97']; ?></label>
		    		<input type="text" name="host" placeholder="<?php echo $txt['txt97']; ?>" value="Hytale Demo Server" maxlength="30" disabled>
		    		<small><?php echo $txt['txt98']; ?></small>
		  		</div>
		  		<div class="required field">
		    		<label><?php echo $txt['txt99']; ?></label>
		    		<input type="text" name="port" placeholder="<?php echo $txt['txt99']; ?>" value="25565" maxlength="6" disabled>
		  		</div>
			...
			<form class="ui small form" action="#<?php validate('edit'); ?>" enctype="multipart/form-data" method="post">
				<h4 class="ui dividing header"><i class="pencil alternate icon"></i> <?php echo $txt['txt93']; ?></h4>
		  		...
		  		<div class="required field">
		    		<label><?php echo $txt['txt97']; ?></label>
		    		<input type="text" name="host" placeholder="<?php echo $txt['txt97']; ?>" value="<?php echo $host; ?>" maxlength="30" disabled>
		    		<small><?php echo $txt['txt98']; ?></small>
		  		</div>
		  		<div class="required field">
		    		<label><?php echo $txt['txt99']; ?></label>
		    		<input type="text" name="port" placeholder="<?php echo $txt['txt99']; ?>" value="<?php echo $port; ?>" maxlength="6" disabled>
		  		</div>
					...
					<h4 class="ui dividing header">Edit Design</h4>
			  		...
			  		<div class="field">
			    		...
			    		<select name="bgImage">
			    			...
			    			<option value="1"<?php if ($info['bgImage'] == 1) { ?> selected <?php } ?>>Minecraft NPCs</option>
			    			<option value="2"<?php if ($info['bgImage'] == 2) { ?> selected <?php } ?>>Minecraft Steve</option>
			    			<option value="3"<?php if ($info['bgImage'] == 3) { ?> selected <?php } ?>>Minecraft Monsters</option>
			    		</select>
			  		</div>
			  		<div class="field">
			    		...
			    		<select name="gradient">
			    			<?php
			    			$grads = array('None', 'Lines', 'Always Grey', 'Arches', 'Asfalt Dark', 'AZ Subtle', 'Binding Dark', 'Black Mamba', 'Black Scales', 'Black Thread', 'Black Thread Light', 'Black Twill', 'Blue Stripes', 'Brick Wall', 'Bright Squares', 'Brilliant', 'Carbon Fibre', 'Cartographer', 'Dark Circles', 'Dark Matter', 'Dimension', 'Escheresque', 'Minecraft Mold', 'Minecraft Bricks', 'Minecraft Creepers');
					...
					<h4 class="ui dividing header">Add Design</h4>
			  		...
			  		<div class="field">
			    		...
			    		<select name="bgImage">
			    			...
			    			<option value="1" selected>Minecraft NPCs</option>
			    			<option value="2">Minecraft Steve</option>
			    			<option value="3">Minecraft Monsters</option>
			    		</select>
			  		</div>
			  		<div class="field">
			    		<label>Background Gradients</label>
...
<footer class="ui container">
	...
	<div class="line">&copy; <?php echo SITE_name; ?> - <?php echo date('d M Y'); ?>. <?php echo $txt['txt167']; ?></div>
	<div class="line">
		<a href="<?php echo $txt['SITE_LINK']; ?>/terms"><?php echo $txt['txt168']; ?></a> 
		<a href="<?php echo $txt['SITE_LINK']; ?>/privacy"><?php echo $txt['txt169']; ?></a> 
		<a href="<?php echo $txt['SITE_LINK']; ?>/help"><?php echo $txt['txt170']; ?></a> 
		<a href="<?php echo $txt['SITE_LINK']; ?>/contacts"><?php echo $txt['txt171']; ?></a> 
	</div>
	<div class="line social">
	<?php
	if (facebookLink) { echo '<a href="'.facebookLink.'" aria-label="Facebook Page" title="Facebook Page"><i class="facebook icon"></i></a>'; }
	if (twitterLink) { echo '<a href="'.twitterLink.'" aria-label="Twitter Page" title="Twitter Page"><i class="twitter icon"></i></a>'; }
	if (googleLink) { echo '<a href="'.googleLink.'" aria-label="Google+ Page" title="Google+ Page"><i class="google plus icon"></i></a>'; }
	if (instagramLink) { echo '<a href="'.instagramLink.'" aria-label="Instagram Page" title="Instagram Page"><i class="instagram icon"></i></a>'; }
	if (youtubeLink) { echo '<a href="'.youtubeLink.'" aria-label="Youtube Channel" title="Youtube Channel"><i class="youtube icon"></i></a>'; }
	if (vkLink) { echo '<a href="'.vkLink.'" aria-label="VK.com Page" title="VK.com Page"><i class="vk icon"></i></a>'; }
	?>
	</div>
</footer>
</div>
